...
def train(args,epochs, encoder_model, decoder_model, model,
    ...
    for i in range(int(args.epochs)):
        ...
        model.fit([encoder_input_data, decoder_input_data, encoder_input_images], decoder_target_data, batch_size=args.batch_size,
        ...
        if (i + 1) % 10 == 0:
            ...
            for seq_index in range(len(val_tokens)):
                ...
                decoded_sentence = decode_sequence(encoder_model, decoder_model, reverse_target_char_index, input_seq,
                                                   encoder_input_images[seq_index])
            ...
            nltk_bleu = nltk.translate.bleu_score.corpus_bleu(src_new, trg_new)
            # print("test: ", nltk_bleu)

    writer.close()
    # predict(test_tokens, test_questions,
...
def predict(test_tokens, test_questions, encoder_model, decoder_model,
            reverse_target_char_index, encoder_input_images_test):
    # print("\nTesting...")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--question', default="what happended?",
                        help='input question.')
    parser.add_argument('--imag', default="dataset/test/synpic53988.jpg",
                        help='input image')
    ...
    # test_images, test_questions, test_answers = Utility.read_predict(args.question, args.imag)
    ...
    #       encoder_input_images, encoder_input_images_val,
    #       decoder_input_data, decoder_target_data, val_questions, val_answers, val_tokens)
    test_images, test_questions, test_answers = Utility.read_predict(" ".join(args.question.split("_")), args.imag)
    # for test_ans in test_questions:
    predict_tokens=[[]]
    for word in test_questions[0].split():
        if word in input_token_index:
            predict_tokens[0].append(input_token_index[word])
        else:
            predict_tokens[0].append(random.randint(0,len(input_token_index)-1))
    test_tokens = np.array(predict_tokens)
    # test_tokens = np.array([[input_token_index[i] for i in test_ans.split()]
    #                         for test_ans in test_questions])
    predict(test_tokens, test_questions, encoder_model, decoder_model,
            reverse_target_char_index, encoder_input_images_test)