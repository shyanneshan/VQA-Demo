...
def move_to_cuda(sample):
    ...
    def _move_to_cuda(maybe_tensor):
        if torch.is_tensor(maybe_tensor):
            return maybe_tensor#.cuda()   hello
...
def tfidf_loading(use_tfidf, w_emb, args):
    if use_tfidf:
        ...
        dict = dataset_RAD.Dictionary.load_from_file(os.path.join(args.dataset, 'dictionary.pkl'))
        ...
        if os.path.isfile(os.path.join(args.dataset, 'embed_tfidf_weights.pkl')) == True:
            # print("Loading embedding tfidf and weights from file")
            with open(os.path.join(args.dataset, 'embed_tfidf_weights.pkl'), 'rb') as f:
                w_emb = torch.load(f)
            # print("Load embedding tfidf and weights from file successfully")
        else:
            # print("Embedding tfidf and weights haven't been saving before")
            tfidf, weights = dataset_RAD.tfidf_from_questions(['train'], args, dict)
            w_emb.init_embedding(os.path.join(args.dataset, 'glove6b_init_300d.npy'), tfidf, weights)
            with open(os.path.join(args.dataset, 'embed_tfidf_weights.pkl'), 'wb') as f:
                torch.save(w_emb, f)
            # print("Saving embedding with tfidf and weights successfully")
    return w_emb
