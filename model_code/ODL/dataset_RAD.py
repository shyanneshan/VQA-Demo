...
class VQAPredict(Dataset):
    def __init__(self, args, dictionary,question_len=12):
        super(VQAPredict, self).__init__()
        self.args = args
        self.question=" ".join(args.question.split("_"))
        self.imag_path=args.imag
        ...
        if args.maml:
            # TODO: load images
            self.maml_images_data = self.create_84()
        # load image data for Auto-encoder module
        if args.autoencoder:
            # TODO: load images
            self.ae_images_data = self.create_128()
        # tokenization
    ...
    def create_84(self):
        img = Image.open(self.imag_path)
        # img = cv2.imread(img_path)
        img = img.resize((84, 84), Image.ANTIALIAS)
        img = img.convert('L')
        transf = transforms.ToTensor()
        img_tensor = transf(img)
        res = torch.Tensor(img_tensor)
        return res
    ...
    def tokenize(self, max_length=12):
        ...
        """
        tokens = self.dictionary.tokenize(self.question, False)
        ...
        self.question=tokens

    def tensorize(self):
        if self.args.maml:
            if type(self.maml_images_data)==numpy.ndarray:
    ...
    def __getitem__(self, index):
        ...
        image_data = [0, 0]
        if self.args.maml:
            maml_images_data = self.maml_images_data
            image_data[0] = maml_images_data
        if self.args.autoencoder:
            ae_images_data = self.ae_images_data
            image_data[1] = ae_images_data

        return image_data, self.question
    ...
    def tensorize(self):
        if self.args.maml:
            if type(self.maml_images_data)==numpy.ndarray:
                self.maml_images_data = torch.from_numpy(self.maml_images_data)
            self.maml_images_data = self.maml_images_data.type('torch.FloatTensor')
            # self.maml_images_data=self.maml_images_data.view(-1,84,84,3)
        if self.args.autoencoder:
            if type(self.ae_images_data) == numpy.ndarray:
                self.ae_images_data = torch.from_numpy(self.ae_images_data)
            self.ae_images_data = self.ae_images_data.type('torch.FloatTensor')
...
def tfidf_from_questions(names,args, dictionary, dataroot='data', target=['rad']):
    ...
    emb_dim = 300
    glove_file = os.path.join('/data2/entity/bhy/VQADEMO/model_code/ODL/glove/glove.6B.%dd.txt' % emb_dim)
...
if __name__=='__main__':
    ...
    tfidf, weights = tfidf_from_questions(['train'], None, dictionary,"data_RAD")
